...
const savedDB = {
  getAll: () => _savedStore,
  save: (idea) => {
    const e = { ...idea, savedId: Date.now() + Math.random(), savedAt: new Date().toISOString() };
    _savedStore = [e, ..._savedStore];
    savedListeners.forEach(fn => fn([..._savedStore]));
    return e;
  },
  remove: (savedId) => {
    _savedStore = _savedStore.filter(i => i.savedId !== savedId);
    savedListeners.forEach(fn => fn([..._savedStore]));
  },
...
const labelSx = {
  ...
  letterSpacing:"0.1em", textTransform:"uppercase", color:C.muted, marginBottom:7,
};
...
// ── Editable Field ─────────────────────────────────────────────
function EditableField({ value, onChange, multiline=false, style={}, canEdit, onLockedClick }) {
  ...
  if (!canEdit) {
    return (
      <div onClick={onLockedClick} style={{ position:"relative", cursor:"pointer", borderRadius:6, padding:"6px 8px", margin:"-6px -8px" }}
  ...
  if (editing) {
    const commonProps = {
      ...
      style:{ ...style, width:"100%", background:C.bg, border:`1px solid ${C.gold}`, borderRadius:8, color:C.text, padding:"10px 12px", fontSize:"inherit", fontFamily:"inherit", fontStyle:"inherit", fontWeight:"inherit", outline:"none", resize:"vertical", boxSizing:"border-box", lineHeight:"inherit" },
  ...
  return (
    <div onClick={() => setEditing(true)} style={{ position:"relative", cursor:"text", borderRadius:6, padding:"6px 8px", margin:"-6px -8px", border:"1px solid transparent" }}
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:400, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ position:"relative", background:C.surface, border:`1px solid ${C.gold}50`, borderRadius:20, padding:"40px 36px", maxWidth:440, width:"100%", textAlign:"center" }}>
        ...
        <h2 style={{ margin:"12px 0 10px", fontSize:24, fontWeight:800, color:C.white }}>Edit Mode is Agency-only</h2>
        <p style={{ margin:"0 0 28px", fontSize:15, color:C.muted, lineHeight:1.7 }}>Agency subscribers can edit every word of their generated concepts — hook, body copy, CTA, and concept name — all inline, instantly.</p>
        <div style={{ background:C.faint, borderRadius:12, padding:16, marginBottom:28, textAlign:"left" }}>
          {["✎ Edit concept name, hook, body & CTA inline","🔄 Re-score after your edits","⬇ Export your custom copy to PDF","🔖 Save edited versions to your library","👥 5 team seats included"].map(f => (
            <div key={f} style={{ fontSize:14, color:"#c0c0d8", padding:"6px 0", borderBottom:`1px solid ${C.border}` }}>{f}</div>
          ))}
        </div>
        <button onClick={onUpgrade} style={{ width:"100%", padding:15, background:`linear-gradient(135deg,${C.gold},${C.amber})`, border:"none", borderRadius:10, color:"#000", fontSize:15, fontWeight:800, cursor:"pointer", fontFamily:"inherit", marginBottom:12 }}>Upgrade to Agency — $99/mo</button>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:system-ui,sans-serif;background:#05050a;color:#f0f0f8;padding:48px;max-width:860px;margin:0 auto}
  .header{border-bottom:2px solid #6c63ff;padding-bottom:32px;margin-bottom:40px}
  h1{font-size:32px;font-weight:900;color:#fff;margin-bottom:8px}
  ...
  .score-item{font-size:12px}.score-item-lbl{color:#6b6b8a;margin-bottom:4px}
  ...
  .edited-badge{background:#ffd70022;border:1px solid #ffd70040;border-radius:100px;padding:2px 8px;font-size:10px;font-weight:700;color:#ffd700;margin-left:8px}
</style></head><body>
<div class="header">
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:20px">
    <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#6c63ff,#ff4d8d);display:flex;align-items:center;justify-content:center;font-size:18px;color:#fff">⚡</div>
    <span style="font-size:22px;font-weight:900;color:#fff">AdSpark</span>
  </div>
...
${ideas.map((idea,i) => {
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px">
    <div>
      <div style="font-size:10px;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:${accent};margin-bottom:6px">Concept ${i+1}${idea._edited?'<span class="edited-badge">EDITED</span>':''}</div>
      <div style="font-size:20px;font-weight:800;color:#fff">${idea.concept}</div>
    </div>
    <div style="text-align:center;min-width:60px">
      <div style="font-size:28px;font-weight:900;color:${sc(overall)}">${overall}</div>
      <div style="font-size:11px;color:${sc(overall)}">${sl(overall)}</div>
    </div>
  </div>
  ...
  <div class="cta-box"><span style="font-size:10px;font-weight:700;text-transform:uppercase;color:#333">CTA</span><span style="font-size:14px;font-weight:700;color:${accent}">${idea.cta}</span></div>
  <div class="scores-grid">${[["Clarity",s.clarity,"#00d4ff"],["Emotional Pull",s.emotion,"#ff4d8d"],["CTA Strength",s.cta_strength,"#ffb830"],["Brand Fit",s.brand_fit,"#b8ff57"]].map(([l,v,c])=>`<div class="score-item"><div class="score-item-lbl">${l}</div><div style="font-weight:700;color:${c}">${v}/100</div><div class="score-bar"><div class="score-bar-fill" style="width:${v}%;background:${c}"></div></div></div>`).join("")}</div>
</div>`;
}).join("")}
<div class="footer"><span class="footer-text">Generated by AdSpark AI</span><span class="footer-text">${ideas.length} concepts • ${new Date().toLocaleDateString()}</span></div>
</body></html>`;
  const blob = new Blob([html], { type:"text/html" });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement("a");
  a.href=url; a.download=`adspark-${(brandInfo.company||"concepts").toLowerCase().replace(/\s+/g,"-")}.html`;
...
function IdeaCard({ idea: initialIdea, index, brandVoice, brandInfo, onSave, plan, onShowUpsell }) {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(`${idea.concept}\n\n${idea.hook}\n\n${idea.body}\n\nCTA: ${idea.cta}`);
    setCopied(true); setTimeout(() => setCopied(false), 2000);
  };

  const handleSave = () => {
    savedDB.save({ ...idea, brandInfo });
  ...
  const handleTab = (t) => {
    setTab(t);
    if (t==="why" && !whyData && !loadingWhy) fetchWhy();
    if (t==="platforms") formatPlatform(activePlatform);
  };

  return (
    <div style={{ background:C.surface, border:`1px solid ${canEdit&&idea._edited?C.gold:C.border}`, borderTop:`2px solid ${canEdit&&idea._edited?C.gold:accent}`, borderRadius:14, overflow:"hidden" }}>
      ...
      {canEdit && (
        <div style={{ background:C.gold+"11", borderBottom:`1px solid ${C.gold}22`, padding:"8px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <span style={{ fontSize:12, color:C.gold, fontWeight:600 }}>✦ Agency Edit Mode — click any field to edit</span>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {idea._edited && (
              <button onClick={rescore} disabled={rescoring} style={{ background:C.gold+"22", border:`1px solid ${C.gold}50`, borderRadius:6, color:C.gold, padding:"4px 10px", fontSize:11, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:5 }}>
                {rescoring ? <><Spinner color={C.gold}/> Rescoring...</> : "🔄 Re-score"}
              </button>
            )}
      ...
      <div style={{ padding:"20px 24px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
          <div style={{ display:"flex", alignItems:"center", gap:14, flex:1, minWidth:0 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <EditableField value={idea.concept} onChange={v => updateField("concept",v)} canEdit={canEdit} onLockedClick={onShowUpsell} style={{ fontSize:17, fontWeight:700, color:C.white, display:"block" }}/>
              <span style={{ fontSize:12, color:C.muted, display:"block", marginTop:2 }}>Score: <span style={{ color:overall>=80?C.lime:overall>=60?C.amber:C.pink, fontWeight:700 }}>{overall>=80?"Excellent":overall>=60?"Good":"Needs work"}</span></span>
            </div>
          </div>
        ...
        <div style={{ display:"flex", borderBottom:`1px solid ${C.border}` }}>
          {[["copy","📝 Copy"],["platforms","📱 Platforms"],["why","🧠 Why It Works"]].map(([id,label]) => (
            <button key={id} onClick={() => handleTab(id)} style={{ padding:"10px 16px", background:"transparent", border:"none", borderBottom:`2px solid ${tab===id?accent:"transparent"}`, color:tab===id?accent:C.muted, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit", marginBottom:-1 }}>{label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding:"20px 24px 24px" }}>
        {tab==="copy" && (
          <div>
            <p style={{ margin:"0 0 6px", fontSize:10, fontWeight:700, color:"#333", letterSpacing:"0.08em", textTransform:"uppercase" }}>Hook</p>
            <div style={{ marginBottom:14 }}>
              <EditableField value={idea.hook} onChange={v => updateField("hook",v)} multiline canEdit={canEdit} onLockedClick={onShowUpsell} style={{ fontSize:15, fontWeight:600, color:"#ccc", fontStyle:"italic", lineHeight:1.5, display:"block" }}/>
            </div>
            <p style={{ margin:"0 0 6px", fontSize:10, fontWeight:700, color:"#333", letterSpacing:"0.08em", textTransform:"uppercase" }}>Body Copy</p>
            <div style={{ marginBottom:14 }}>
              <EditableField value={idea.body} onChange={v => updateField("body",v)} multiline canEdit={canEdit} onLockedClick={onShowUpsell} style={{ fontSize:13, color:C.muted, lineHeight:1.7, display:"block" }}/>
            </div>
            ...
            <div style={{ background:C.faint, borderRadius:8, padding:"10px 14px", display:"flex", gap:10, alignItems:"center" }}>
              ...
              <EditableField value={idea.cta} onChange={v => updateField("cta",v)} canEdit={canEdit} onLockedClick={onShowUpsell} style={{ fontSize:13, fontWeight:700, color:accent, display:"block", flex:1 }}/>
            </div>
            {!canEdit && (
              <button onClick={onShowUpsell} style={{ marginTop:14, width:"100%", background:C.gold+"11", border:`1px solid ${C.gold}30`, borderRadius:8, color:C.gold, padding:"10px", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
            ...
            {loadingPlat ? (
              ...
              <div>
                ...
                {platformCopy[activePlatform].tips && (
                  <div style={{ display:"flex", gap:8, background:C.electric+"11", border:`1px solid ${C.electric}30`, borderRadius:8, padding:"10px 14px", marginBottom:12 }}>
        ...
        {tab==="why" && (
          <div>
            {loadingWhy ? (
              <div style={{ display:"flex", alignItems:"center", gap:10, color:C.muted, fontSize:14 }}><Spinner color={C.pink}/> Analyzing psychology...</div>
            ) : whyData ? (
...
function SavedLibrary({ onClose, plan }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:200, display:"flex" }}>
      ...
      <div style={{ position:"relative", marginLeft:"auto", width:"min(520px,100vw)", height:"100vh", background:C.surface, borderLeft:`1px solid ${C.border}`, display:"flex", flexDirection:"column" }}>
        <div style={{ padding:"24px 24px 16px", borderBottom:`1px solid ${C.border}` }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={onClose} style={{ background:C.faint, border:`1px solid ${C.border}`, borderRadius:8, color:C.muted, padding:"8px 12px", fontSize:18, cursor:"pointer", fontFamily:"inherit" }}>✕</button>
            </div>
          </div>
          {saved.length > 0 && (
            <input value={filter} onChange={e => setFilter(e.target.value)} placeholder="Filter by concept or brand..." style={{ width:"100%", background:C.bg, border:`1px solid ${C.border}`, borderRadius:8, color:C.text, padding:"9px 12px", fontSize:13, fontFamily:"inherit", outline:"none", boxSizing:"border-box" }}/>
          )}
        </div>
              ...
              <div key={idea.savedId} style={{ background:C.bg, border:`1px solid ${idea._edited?C.gold+"50":C.border}`, borderRadius:12, padding:16, marginBottom:12 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginLeft:10, flexShrink:0 }}>
                    ...
                    <button onClick={() => savedDB.remove(idea.savedId)} style={{ background:"transparent", border:"none", color:"#444", fontSize:16, cursor:"pointer", padding:"2px 4px" }}>✕</button>
                  </div>
                </div>
...
// ══════════════════════════════════════════════════════════════
// MAIN APP — all sub-components are defined OUTSIDE this function
// to prevent re-creation on every render (fixes keyboard bug)
// ══════════════════════════════════════════════════════════════
export default function AdSpark() {
  const [section, setSection]           = useState("home"); // home | app | product | pricing | about
  const [company, setCompany]           = useState("");
  const [industry, setIndustry]         = useState("");
  const [product, setProduct]           = useState("");
  const [audience, setAudience]         = useState("");
  const [tone, setTone]                 = useState("Bold & Edgy");
  const [format, setFormat]             = useState("Full Campaign Concept");
  ...
  // KEY FIX: individual state fields instead of a form object
  // prevents full re-render on every keystroke
  const form = { company, industry, product, audience, tone, format };

  const showToastMsg = (msg) => { setToast(msg); setTimeout(() => setToast(""), 2500); };

  const generate = async () => {
    if (!company || !product || !audience) { setError("Fill in company, product, and audience to continue."); return; }
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"system-ui,sans-serif", minHeight:"100vh" }}>
      ...
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"rgba(5,5,10,0.9)", backdropFilter:"blur(16px)", borderBottom:`1px solid ${C.border}`, padding:"0 24px" }}>
        <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:60 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ fontSize:17, fontWeight:800, color:C.white }}>AdSpark</span>
            <span style={{ fontSize:10, fontWeight:700, background:C.lime+"22", border:`1px solid ${C.lime}40`, color:C.lime, borderRadius:100, padding:"2px 8px" }}>v4</span>
          </div>
          <div style={{ display:"flex", gap:24, alignItems:"center" }}>
            {[["Product","product"],["Pricing","pricing"],["About","about"]].map(([label,target]) => (
              <button key={label} onClick={() => setSection(target)} style={{ background:"transparent", border:"none", color:C.muted, fontSize:14, fontWeight:500, cursor:"pointer", fontFamily:"inherit", padding:0, transition:"color .15s" }}
      ...
      <section style={{ padding:"100px 24px 80px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"relative", maxWidth:800, margin:"0 auto" }}>
          <div style={{ marginBottom:20 }}><Tag color={C.gold}>✦ Agency full edit mode — now live</Tag></div>
          ...
          <p style={{ margin:"0 auto 40px", maxWidth:560, fontSize:"clamp(16px,2.5vw,20px)", color:C.muted, lineHeight:1.7 }}>
            Generate. Score. Edit. Format. Understand. Save. Export. Everything to run smarter ads — in one place.
          </p>
          ...
          <div style={{ marginTop:48, display:"flex", gap:20, justifyContent:"center", flexWrap:"wrap" }}>
            {["AI scoring","Platform formatter","Psychology explainer","Saved library","PDF export","✦ Full edit mode"].map((t,i) => (
              <span key={i} style={{ fontSize:13, color:t.includes("✦")?C.gold:C.muted, display:"flex", alignItems:"center", gap:6 }}>
                <span style={{ color:t.includes("✦")?C.gold:C.lime }}>✓</span>{t}
              </span>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding:"90px 24px" }}>
        <div style={{ maxWidth:1060, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:20 }}>
            {[
              { icon:"📊", color:C.cyan,    title:"Performance Scoring",  body:"Scored across clarity, emotion, CTA strength, and brand fit.",   tier:null },
              { icon:"📱", color:C.lime,    title:"Platform Formatter",   body:"Auto-reformat for Instagram, Google Ads, LinkedIn, and TikTok.", tier:null },
              ...
              { icon:"⬇",  color:C.electric,title:"PDF Export",           body:"Export any concept or your full library as a branded PDF.",      tier:"Pro+" },
              { icon:"✎",  color:C.gold,    title:"Full Edit Mode",       body:"Edit every word inline, then re-score your changes.",            tier:"Agency" },
              ...
              <div key={title} style={{ background:C.surface, border:`1px solid ${tier==="Agency"?C.gold+"40":C.border}`, borderRadius:14, padding:28, position:"relative" }}>
                {tier && <div style={{ position:"absolute", top:14, right:14, background:tier==="Agency"?C.gold+"22":C.electric+"22", border:`1px solid ${tier==="Agency"?C.gold+"50":C.electric+"50"}`, borderRadius:100, padding:"2px 8px", fontSize:10, fontWeight:700, color:tier==="Agency"?C.gold:C.electric }}>{tier}</div>}
      ...
      <section style={{ padding:"80px 24px", background:C.surface, borderTop:`1px solid ${C.border}`, borderBottom:`1px solid ${C.border}` }}>
        <div style={{ maxWidth:960, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:20 }}>
            {[
              { name:"Starter", price:"Free", per:"forever", key:"starter", cta:"Get started",      features:["10 concepts/month","Performance scoring","Copy to clipboard","10 saved ideas"] },
              { name:"Pro",     price:"$29",  per:"/month",  key:"pro",     cta:"Start free trial", features:["Unlimited concepts","All platform formatters","Psychology explainer","Unlimited library","PDF exports"] },
              { name:"Agency",  price:"$99",  per:"/month",  key:"agency",  cta:"Start Agency trial",features:["Everything in Pro","✦ Full inline edit mode","🔄 Re-score after edits","5 team seats","API access"] },
              ...
              return (
                <div key={name} style={{ background:isAgency?`linear-gradient(160deg,${C.gold}18,${C.amber}0a)`:C.bg, border:`1px solid ${isAgency?C.gold+"60":C.border}`, borderRadius:16, padding:32, position:"relative" }}>
                  {isAgency && <div style={{ position:"absolute", top:16, right:16, background:`linear-gradient(135deg,${C.gold},${C.amber})`, borderRadius:100, padding:"3px 10px", fontSize:10, fontWeight:700, color:"#000" }}>✦ AGENCY</div>}
                  ...
                  <ul style={{ listStyle:"none", margin:"0 0 28px", padding:0, display:"flex", flexDirection:"column", gap:10 }}>
                    {features.map(f => (
                      <li key={f} style={{ fontSize:14, color:"#b0b0c8", display:"flex", gap:8 }}>
      ...
      <footer style={{ borderTop:`1px solid ${C.border}`, padding:"36px 24px" }}>
        <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:20 }}>
          ...
          <div style={{ display:"flex", gap:24 }}>
            {["Privacy","Terms","Contact"].map(l => <a key={l} href="#" onClick={e=>e.preventDefault()} style={{ fontSize:13, color:C.muted, textDecoration:"none" }}>{l}</a>)}
          </div>
        </div>
      </footer>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}*{box-sizing:border-box}input::placeholder,textarea::placeholder{color:#2a2a3e}`}</style>
    </div>
  );
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"system-ui,sans-serif", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"80px 24px" }}>
        <div style={{ textAlign:"center", marginBottom:72 }}>
          <Tag color={C.electric}>The Product</Tag>
          <h1 style={{ margin:"20px 0 16px", fontSize:"clamp(32px,5vw,60px)", fontWeight:900, letterSpacing:"-0.03em", color:C.white }}>Everything AdSpark does</h1>
          <p style={{ color:C.muted, fontSize:18, maxWidth:500, margin:"0 auto" }}>One tool that takes you from blank page to launch-ready ad copy in under 60 seconds.</p>
        </div>

        {/* Feature deep dives */}
        {[
          { icon:"📊", color:C.cyan,    title:"AI Performance Scoring", tag:"All plans",
            desc:"Every concept is scored 0–100 across four dimensions — Clarity, Emotional Pull, CTA Strength, and Brand Fit. Know which concept to run before you spend a dollar on ads. Scores update in real time when you edit your copy.",
            bullets:["Clarity & readability score","Emotional pull score","CTA strength score","Brand fit score","Animated score ring for quick reference"] },
          { icon:"📱", color:C.lime,    title:"4-Platform Formatter", tag:"All plans",
            desc:"Write once, publish everywhere. AdSpark automatically reformats your concept for Instagram (2,200 chars), Google Ads (90 char limit), LinkedIn (600 chars), and TikTok (150 chars) — hashtags, tone, and length all adapted for each platform.",
            bullets:["Instagram caption with hashtags","Google Ads within character limits","LinkedIn professional tone","TikTok conversational style","Platform-specific tips included"] },
          { icon:"🧠", color:C.pink,    title:"Psychology Explainer", tag:"All plans",
            desc:"AdSpark doesn't just give you copy — it teaches you why it works. Every concept comes with a psychological trigger analysis, the cognitive bias being leveraged, why the hook grabs attention, and why the CTA drives action.",
            bullets:["Psychological trigger identification","Cognitive bias breakdown","Hook attention analysis","CTA conversion reasoning","Honest weakness flagged"] },
          { icon:"🔖", color:C.amber,   title:"Saved Ideas Library", tag:"All plans",
            desc:"Your best concepts deserve to be saved. Bookmark any idea with one click, search and filter your library by brand or concept name, and build up a personal swipe file over time.",
            bullets:["One-click save from any concept","Search by brand or concept name","View score at a glance","Remove ideas you no longer need","Persists across your session"] },
          { icon:"⬇",  color:C.electric,title:"PDF Export", tag:"Pro & Agency",
            desc:"Export any concept — or your entire library — as a beautifully branded HTML/PDF file. Perfect for sharing with clients, presenting to your team, or keeping a record of your best campaigns.",
            bullets:["Export single concepts","Export full library at once","Branded AdSpark design","Score breakdown included","EDITED badge on modified concepts"] },
          { icon:"✎",  color:C.gold,    title:"Full Inline Edit Mode", tag:"Agency only",
            desc:"The most powerful feature AdSpark offers. Agency subscribers can click any field — concept name, hook, body copy, or CTA — and edit it directly inline. After editing, hit Re-score and the AI re-evaluates your performance scores instantly.",
            bullets:["Edit concept name inline","Edit hook inline","Edit body copy inline","Edit CTA inline","Re-score after any edit","EDITED badge tracks changes"] },
        ].map(({ icon,color,title,tag,desc,bullets }) => (
          <div key={title} style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:40, marginBottom:64, alignItems:"start" }}>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
                <div style={{ width:52, height:52, borderRadius:12, background:color+"20", border:`1px solid ${color}40`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:26 }}>{icon}</div>
                <div>
                  <span style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", color:color, display:"block", marginBottom:4 }}>{tag}</span>
                  <h2 style={{ margin:0, fontSize:22, fontWeight:800, color:C.white }}>{title}</h2>
                </div>
              </div>
              <p style={{ margin:0, fontSize:15, color:C.muted, lineHeight:1.8 }}>{desc}</p>
            </div>
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, padding:24 }}>
              {bullets.map(b => (
                <div key={b} style={{ display:"flex", gap:10, alignItems:"flex-start", padding:"8px 0", borderBottom:`1px solid ${C.border}` }}>
                  <span style={{ color, fontSize:14, flexShrink:0 }}>✓</span>
                  <span style={{ fontSize:14, color:"#c0c0d8" }}>{b}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ textAlign:"center", marginTop:40 }}>
          <button onClick={() => setSection("app")} style={{ background:`linear-gradient(135deg,${C.electric},${C.pink})`, border:"none", borderRadius:10, color:"#fff", padding:"15px 36px", fontSize:16, fontWeight:700, cursor:"pointer", boxShadow:`0 0 40px ${C.electric}50` }}>Try AdSpark free →</button>
        </div>
      </div>
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"system-ui,sans-serif", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:960, margin:"0 auto", padding:"80px 24px" }}>
        <div style={{ textAlign:"center", marginBottom:64 }}>
          <Tag color={C.lime}>Pricing</Tag>
          <h1 style={{ margin:"20px 0 16px", fontSize:"clamp(32px,5vw,60px)", fontWeight:900, letterSpacing:"-0.03em", color:C.white }}>Simple, honest pricing</h1>
          <p style={{ color:C.muted, fontSize:18 }}>Start free. No credit card required. Upgrade when you're ready.</p>
        </div>

        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:20, marginBottom:64 }}>
          {[
            { name:"Starter", price:"Free", per:"forever", key:"starter", cta:"Get started free",
              color:C.muted,
              features:["10 concepts per month","Performance scoring","3 ad formats","Copy to clipboard","10 saved ideas","Basic support"] },
            { name:"Pro", price:"$29", per:"/month", key:"pro", cta:"Start 7-day free trial",
              color:C.electric,
              features:["Unlimited concepts","All 6 ad formats","All 6 tones","4-platform formatter","Psychology explainer","Unlimited saved library","Unlimited PDF exports","Brand voice learning","Email support"] },
            { name:"Agency", price:"$99", per:"/month", key:"agency", cta:"Start Agency trial",
              color:C.gold,
              features:["Everything in Pro","✦ Full inline edit mode","🔄 Re-score after edits","5 team seats","White-label PDF exports","API access","Priority support","Dedicated onboarding"] },
          ].map(({ name,price,per,key,cta,color,features }) => {
            const isAgency = key==="agency";
            const isPro = key==="pro";
            return (
              <div key={name} style={{ background:isAgency?`linear-gradient(160deg,${C.gold}18,${C.amber}0a)`:isPro?`linear-gradient(160deg,${C.electric}15,${C.pink}08)`:C.surface, border:`1px solid ${isAgency?C.gold+"60":isPro?C.electric+"50":C.border}`, borderRadius:18, padding:36, position:"relative" }}>
                {isAgency && <div style={{ position:"absolute", top:18, right:18, background:`linear-gradient(135deg,${C.gold},${C.amber})`, borderRadius:100, padding:"3px 12px", fontSize:10, fontWeight:700, color:"#000" }}>✦ BEST VALUE</div>}
                {isPro && <div style={{ position:"absolute", top:18, right:18, background:`linear-gradient(135deg,${C.electric},${C.pink})`, borderRadius:100, padding:"3px 12px", fontSize:10, fontWeight:700, color:"#fff" }}>POPULAR</div>}
                <div style={{ fontSize:14, fontWeight:700, color, marginBottom:8 }}>{name}</div>
                <div style={{ display:"flex", alignItems:"baseline", gap:4, marginBottom:8 }}>
                  <span style={{ fontSize:48, fontWeight:900, color:C.white, letterSpacing:"-0.04em" }}>{price}</span>
                  <span style={{ fontSize:14, color:C.muted }}>{per}</span>
                </div>
                {key !== "starter" && <p style={{ margin:"0 0 24px", fontSize:13, color:C.muted }}>Cancel anytime. No hidden fees.</p>}
                {key === "starter" && <p style={{ margin:"0 0 24px", fontSize:13, color:C.muted }}>No credit card required.</p>}
                <ul style={{ listStyle:"none", margin:"0 0 32px", padding:0, display:"flex", flexDirection:"column", gap:11 }}>
                  {features.map(f => (
                    <li key={f} style={{ fontSize:14, color:"#c0c0d8", display:"flex", gap:8, alignItems:"flex-start" }}>
                      <span style={{ color, flexShrink:0 }}>✓</span>{f}
                    </li>
                  ))}
                </ul>
                <button onClick={() => { setPlan(key); setSection("app"); }} style={{ width:"100%", padding:14, background:isAgency?`linear-gradient(135deg,${C.gold},${C.amber})`:isPro?`linear-gradient(135deg,${C.electric},${C.pink})`:"transparent", border:`1px solid ${isAgency||isPro?"transparent":C.border}`, borderRadius:10, color:isAgency?"#000":"#fff", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>{cta}</button>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ maxWidth:640, margin:"0 auto" }}>
          <h2 style={{ margin:"0 0 32px", fontSize:28, fontWeight:800, color:C.white, textAlign:"center" }}>Frequently asked questions</h2>
          {[
            { q:"Can I cancel anytime?", a:"Yes. Cancel with one click from your account settings. No questions asked, no cancellation fees." },
            { q:"Do I need a credit card to start?", a:"No. The Starter plan is completely free with no credit card required. You only need a card when upgrading to Pro or Agency." },
            { q:"What is the Agency edit mode?", a:"Agency subscribers can click any field in a generated concept — the name, hook, body copy, or CTA — and edit it directly. After editing you can re-score the concept with AI." },
            { q:"Can I switch plans?", a:"Yes, you can upgrade or downgrade at any time. If you upgrade, the change is immediate. If you downgrade, it takes effect at the next billing cycle." },
            { q:"What payment methods do you accept?", a:"We accept all major credit and debit cards through Stripe — Visa, Mastercard, American Express, and Discover." },
          ].map(({ q,a }) => (
            <div key={q} style={{ borderBottom:`1px solid ${C.border}`, padding:"20px 0" }}>
              <h3 style={{ margin:"0 0 10px", fontSize:16, fontWeight:700, color:C.white }}>{q}</h3>
              <p style={{ margin:0, fontSize:14, color:C.muted, lineHeight:1.7 }}>{a}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"system-ui,sans-serif", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:760, margin:"0 auto", padding:"80px 24px" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:80 }}>
          <Tag color={C.pink}>Our story</Tag>
          <h1 style={{ margin:"20px 0 20px", fontSize:"clamp(32px,5vw,56px)", fontWeight:900, letterSpacing:"-0.03em", color:C.white, lineHeight:1.1 }}>
            We built the tool<br/>we always needed
          </h1>
          <p style={{ fontSize:18, color:C.muted, lineHeight:1.8, maxWidth:580, margin:"0 auto" }}>
            AdSpark was born from a simple frustration — writing ad copy is time-consuming, expensive, and hit-or-miss. We believed there had to be a better way. So we built one.
          </p>
        </div>

        {/* Values */}
        <div style={{ marginBottom:80 }}>
          <h2 style={{ margin:"0 0 32px", fontSize:28, fontWeight:800, color:C.white }}>What we believe</h2>
          {[
            { icon:"⚡", title:"Speed without sacrifice", body:"Great creative shouldn't take days. With the right tools, a founder or marketer can produce agency-quality concepts in under a minute. That's what we built." },
            { icon:"🧠", title:"Understanding beats copying", body:"We don't just give you copy. We explain the psychology behind it so you get smarter every time you use AdSpark. Knowledge compounds." },
            { icon:"💰", title:"Affordable for everyone", body:"Small businesses deserve the same quality tools as Fortune 500 companies. That's why we start at free and keep our pricing honest and transparent." },
            { icon:"🎯", title:"Results over features", body:"We add features only when they solve a real problem. AdSpark is deliberately focused — we'd rather do six things exceptionally well than fifty things poorly." },
          ].map(({ icon,title,body }) => (
            <div key={title} style={{ display:"flex", gap:20, marginBottom:32, padding:24, background:C.surface, border:`1px solid ${C.border}`, borderRadius:14 }}>
              <div style={{ fontSize:28, flexShrink:0, width:48, height:48, background:C.faint, borderRadius:10, display:"flex", alignItems:"center", justifyContent:"center" }}>{icon}</div>
              <div>
                <h3 style={{ margin:"0 0 8px", fontSize:17, fontWeight:700, color:C.white }}>{title}</h3>
                <p style={{ margin:0, fontSize:14, color:C.muted, lineHeight:1.7 }}>{body}</p>
              </div>
            </div>
          ))}
        ...
        {/* Stats */}
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:40, marginBottom:80, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:32, textAlign:"center" }}>
          {[["14,200+","Concepts generated"],["3,100+","Brands served"],["94%","Satisfaction rate"],["2026","Founded"]].map(([val,label]) => (
            <div key={label}>
              <div style={{ fontSize:32, fontWeight:900, color:C.white, letterSpacing:"-0.03em" }}>{val}</div>
        ...
        {/* Contact */}
        <div style={{ background:`linear-gradient(135deg,${C.electric}20,${C.pink}15)`, border:`1px solid ${C.electric}40`, borderRadius:20, padding:"48px 40px", textAlign:"center" }}>
          <h2 style={{ margin:"0 0 14px", fontSize:28, fontWeight:800, color:C.white }}>Get in touch</h2>
          <p style={{ margin:"0 0 28px", fontSize:15, color:C.muted, lineHeight:1.7 }}>
            Have a question, a partnership idea, or just want to say hello?<br/>We read every message personally.
          </p>
          <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap" }}>
            <a href="mailto:hello@adspark.io" style={{ background:`linear-gradient(135deg,${C.electric},${C.pink})`, border:"none", borderRadius:10, color:"#fff", padding:"13px 28px", fontSize:15, fontWeight:700, cursor:"pointer", textDecoration:"none", display:"inline-block" }}>✉ hello@adspark.io</a>
            <button onClick={() => setSection("app")} style={{ background:"transparent", border:`1px solid ${C.border}`, borderRadius:10, color:C.muted, padding:"13px 28px", fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Try the product</button>
          </div>
        </div>
      </div>
      <style>{`*{box-sizing:border-box}`}</style>
  ...
  return (
    <div style={{ background:C.bg, color:C.text, fontFamily:"system-ui,sans-serif", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:800, margin:"0 auto", padding:"52px 24px" }}>
        <button onClick={() => setSection("home")} style={{ background:"transparent", border:"none", color:C.muted, fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", gap:6, fontFamily:"inherit", padding:0, marginBottom:28 }}>← Back to home</button>
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 16px", marginBottom:24, display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          ...
          {Object.entries(PLANS).map(([key,p]) => (
            <button key={key} onClick={() => setPlan(key)} style={{ padding:"5px 14px", borderRadius:100, border:`1px solid ${plan===key?p.color:C.border}`, background:plan===key?p.color+"22":"transparent", color:plan===key?p.color:C.muted, fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
              {key==="agency"&&"✦ "}{p.name}
            </button>
          ))}
        </div>
        ...
        <div style={{ marginBottom:28 }}>
          ...
          <p style={{ margin:0, color:C.muted, fontSize:15 }}>{PLANS[plan]?.canEdit ? "✦ Agency edit mode is ON — click any field to edit." : "Upgrade to Agency to edit concepts inline."}</p>
        </div>

        {/* Form — each field has its own setState to prevent re-renders */}
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:28, marginBottom:24 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18, marginBottom:18 }}>
            <div>
              <label style={labelSx}>Company Name *</label>
              <input value={company} onChange={e => setCompany(e.target.value)} placeholder="e.g. Acme Inc." style={inputSx(false)}/>
            </div>
            <div>
          ...
          <div style={{ marginBottom:18 }}>
            <label style={labelSx}>Product / Service *</label>
            <textarea value={product} onChange={e => setProduct(e.target.value)} placeholder="Describe what you sell and what makes it unique..." rows={3} style={{ ...inputSx(false), resize:"vertical" }}/>
          </div>

          <div style={{ marginBottom:18 }}>
            <label style={labelSx}>Target Audience *</label>
            <input value={audience} onChange={e => setAudience(e.target.value)} placeholder="e.g. Busy parents 30–45 who want healthy meal options" style={inputSx(false)}/>
          </div>
          ...
          <div style={{ marginBottom:18 }}>
            ...
            {showVoice && <textarea value={brandVoice} onChange={e => setBrandVoice(e.target.value)} placeholder="Paste 2-3 examples of copy that matches your brand voice..." rows={3} style={{ ...inputSx(false), resize:"vertical" }}/>}
          </div>

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18, marginBottom:24 }}>
            <div>
              <label style={labelSx}>Tone</label>
              <select value={tone} onChange={e => setTone(e.target.value)} style={{ ...inputSx(false), cursor:"pointer" }}>
                {TONES.map(t => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label style={labelSx}>Ad Format</label>
              <select value={format} onChange={e => setFormat(e.target.value)} style={{ ...inputSx(false), cursor:"pointer" }}>
          ...
          <button onClick={generate} disabled={loading} style={{ width:"100%", padding:15, background:loading?C.faint:`linear-gradient(135deg,${C.electric},${C.pink})`, border:"none", borderRadius:10, color:loading?C.muted:"#fff", fontSize:15, fontWeight:700, cursor:loading?"not-allowed":"pointer", fontFamily:"inherit", boxShadow:loading?"none":`0 0 28px ${C.electric}40` }}>
            {loading
              ? <span style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10 }}><Spinner/> Generating & scoring...</span>
              : "Generate 3 scored concepts →"
            }
          </button>
        </div>
        ...
        {ideas.length > 0 && (
          <div>
            ...
            <p style={{ color:C.muted, fontSize:13, marginBottom:18 }}>
              {PLANS[plan]?.canEdit ? "✦ Click any field to edit. Hit 🔄 Re-score to update performance after edits." : "Click tabs for platforms or psychology. Upgrade to Agency to edit inline."}
            </p>
      ...
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}*{box-sizing:border-box}select option{background:${C.bg}}input::placeholder,textarea::placeholder{color:#2a2a3e}::-webkit-scrollbar{width:6px}::-webkit-scrollbar-thumb{background:${C.border};border-radius:3px}`}</style>
    </div>
  );
}
